...
from genericpath import isdir, isfile, exists
...
async def remove_file(file_list): 
    for file_path in file_list:
        ...
        if (file_suf).lower() in suffix_list["arh"]:
            ...
            while exists(f"{adres_folder_list[adres_folder]()}/{new_name}"):
                ...
                i += 1 
            await shutil.unpack_archive(folder, f"{adres_folder_list[adres_folder]()}/{new_name}")
            await remove(folder)
            ...
            while exists(f"{adres_folder_list['x_files']()}/{new_name}"):
                new_name = (f"({i})" + str(file_name))
                i += 1

            await folder.replace(f"{adres_folder_list['x_files']()}/{new_name}")
            ...
            while exists(f"{adres_folder_list[adres_folder]()}/{new_name}"):
                ...
                i += 1
            await folder.replace(f"{adres_folder_list[adres_folder]()}/{new_name}")
